...
for m in n:
    for j in range(len(m)):
        if m[j].strip() and not m[j].strip().isdigit():
            try:
                conversion_technology[j].append(
                    m[j].strip().replace("[", "").replace("]", "")
                )
            except KeyError:
...
for move in moves:
    conversion_technology[move.target - 1] = [
        x for x in reversed(conversion_technology[move.source - 1][: move.amount])
    ] + conversion_technology[move.target - 1]
    conversion_technology[move.source - 1] = conversion_technology[move.source - 1][
        move.amount :
    ]
